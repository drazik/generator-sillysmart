...
gadash.auth.onUnAuthorizedDefault = function() {
	window.fireEvent('onGAUnauthorized');
...
gadash.auth.onAuthorizedDefault = function() {
	window.fireEvent('onGAAuthorized', {
		googleUser: gadash.userInfo
	});
    var GASection = document.getElementById('sls-bo-section-ga');
    GASection.getElements('.sls-bo-dashboard-module').setStyles({'display':null});
    var authBtn = document.getElementById('authorize-button');
    if (authBtn)
        GASection.removeChild(authBtn);
...
gadash.auth.loadUserNameHander_ = function(response) {
  ...
  if (response.error)
    return gadash.auth.proposeConnection();
...
gadash.auth.proposeConnection = function() {
    var authBtn = document.createElement('button');
    authBtn.id = 'authorize-button';
    authBtn.className = 'sls-bo-color';
    authBtn.innerHTML = slsBuild.langs.SLS_BO_ACTIVATE_GOOGLE_ANALYTICS;
    authBtn.onclick = gadash.auth.authorize;
    authBtn.setStyles({'border': 'none','padding': '0 20px','height': '34px','cursor': 'pointer','font-family': 'GothamBold','display': 'inline-block','margin': '20px auto','border-radius': '2px'});
    var GASection = document.getElementById('sls-bo-section-ga');
    GASection.getElements('.sls-bo-dashboard-module').setStyles({'display':'none'});
    GASection.appendChild(authBtn);
    GASection.style.textAlign = 'center';
}
...
gadash.gviz.lineChart = {
  ...
  'chartOptions': {
    ...
    colors: ['#FE405E', '#B4A294', '#BDCD00', '#F9B200', '#79ADB6', '#A29EDC', '#E45C5C', '#00B0B2', '#79B51C', '#870D56', '#ED9961'],
    /* /HACK SLS */ 
    hAxis: {
      format: 'MMM d',
