<?php
class SLS_BoLogs extends SLS_BoControllerProtected 
{
	...
	public function action()
	{
		// Objects
		$xml = $this->getXML();
		$xml->addFullTag("view_log",$this->_generic->getFullPath("SLS_Bo","LogsApp",array(),false));
		...
		$dates = array();
		$all = array();
		$handle = opendir($this->_generic->getPathConfig("logs"));
		
		// Foreach directories 
		while (false !== ($dir = readdir($handle)))
		{
			if (is_dir($this->_generic->getPathConfig("logs")."/".$dir) && substr($dir, 0, 1) != ".") 
			{				
				$handle2 = opendir($this->_generic->getPathConfig("logs")."/".$dir);				
				while (false !== ($file = readdir($handle2)))
				{					
					if (!is_dir($this->_generic->getPathConfig("logs")."/".$dir."/".$file) && substr($file, 0, 1) != ".") 
					{
