<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:php="http://php.net/xsl" xmlns="http://www.w3.org/1999/xhtml" xmlns:dyn="http://exslt.org/dynamic" extension-element-prefixes="dyn">
	<!-- 
	 	- Function BoEdit
	 	- Generic's generation of back-office editing
	 	- Don't change anything
	-->
	<xsl:template name="BoEdit">
		<div id="sls-bo-fixed-header">
			<div class="sls-bo-fixed-header-content"></div>
		</div>
		<div class="fixed-in-header">
			<h1><span class="sls-bo-color-text">|||sls:lang:SLS_BO_GENERIC_EDIT_RECORDSET|||</span><a href="{//View/page/urls/list}" title=""><xsl:value-of select="//View/page/model/label" /></a></h1>
		</div>

		<div class="sls-bo-form-page main-core-content">
			<xsl:if test="count(//Statics/Sls/Configs/site/langs/name) &gt; 1 and (count(//View/page/columns/column[multilanguage = 'true']) &gt; 0 or count(//View/page/children/child[urls/add/@authorized = 'true' or urls/edit/@authorized = 'true']/columns/column[multilanguage = 'true']) &gt; 0)">
			...
			<form action="" method="post" sls-validation="true">
				...
				<div class="sls-bo-form-page-langs" sls-db="{//View/page/model/db}" sls-model="{//View/page/model/table}" sls-recordset-id="{//View/page/columns/column[pk = 'true' and name != 'pk_lang']/values/value}">
					...
					<xsl:for-each select="//Statics/Sls/Configs/site/langs/name">
						...
						<div class="sls-bo-form-page-lang"  sls-lang="{$lang}">
							...
							<xsl:if test="count(//View/page/columns/column[pk = 'false' and html_type != 'input_textarea' and html_type != 'input_file' and ((multilanguage = 'false' and $isDefaultLang) or multilanguage = 'true')]) &gt; 0 or count(//View/page/bearers/column) &gt; 0">
								<div class="sls-bo-form-page-section form-type-regulars">
									...
									<div class="sls-bo-form-page-section-wrapper">
										<div class="sls-bo-form-page-section-content">
											<xsl:for-each select="//View/page/columns/column[pk = 'false' and html_type != 'input_textarea' and html_type != 'input_file' and ((multilanguage = 'false' and $isDefaultLang) or multilanguage = 'true')]">
															...
															<xsl:when test="html_type = 'input_radio'">
																<xsl:choose>
																	<xsl:when test="(count(choices/choice) &lt;= 4 and required = 'true') or (count(choices/choice) &lt;= 3 and required = 'false')">
																		...
																		<xsl:choose>
																			<xsl:when test="required = 'true' and count(choices/choice) = 2 and count(choices/choice[. = 'true' or . = 'false']) = 2">
																				<div class="toggler-btn toggler-btn-radio horizontal">
																					<div class="toggler-btn-knob"></div>
																					<input type="radio" name="{$fieldName}" sls-toggler-state="on" value="true"><xsl:if test="$fieldValue = 'true'"><xsl:attribute name="checked">checked</xsl:attribute></xsl:if></input>
																					<input type="radio" name="{$fieldName}" sls-toggler-state="off" value="false"><xsl:if test="$fieldValue = 'false'"><xsl:attribute name="checked">checked</xsl:attribute></xsl:if></input>
																				</div>
																			</xsl:when>
																			<xsl:otherwise>
														...
														<div class="sls-input-file-drop-zone" onclick="var rendering = this.getParent('.sls-input-file').getElement('.sls-drop-zone-rendering'); if (!Browser.ie8 &amp;&amp; ((rendering &amp;&amp; !this.getParent('.sls-form-page-field-error')) || !rendering))$('{$fieldId}').click();">
															<xsl:if test="$fieldValue != ''">
																<div class="sls-input-file-params">
																	<input type="hidden" name="{$fieldName}[file]" value="{$fieldValue}">
																		<xsl:choose>
																			<xsl:when test="values/value[@lang = $lang]/@mime != '' and values/value[@lang = $lang]/@img != 'true'"><xsl:attribute name="sls-input-poster"><xsl:value-of select="values/value[@lang = $lang]/@mime" /></xsl:attribute></xsl:when>
															...
															<input type="file" name="{$fieldName}" id="{$fieldId}" sls-input-file-value="{$fieldValue}" sls-type="{specific_type}" sls-type-extended="{specific_type_extended}" sls-file-uid="{file_uid}" sls-required="{required}" onchange="if (this.retrieve('FileUpload')) FileUpload.prototype.submit.call(this.retrieve('FileUpload')); if (Browser.ie8)this.parentNode.submit();">
																<xsl:if test="specific_type_extended = 'img'">
																	...
																	<xsl:if test="values/value[@lang = $lang]/@size != ''"><xsl:attribute name="sls-input-file-size"><xsl:value-of select="values/value[@lang = $lang]/@size" /></xsl:attribute></xsl:if>
																</xsl:if>
															</input>
				...
				<xsl:for-each select="//View/page/children/child[urls/add/@authorized = 'true' or urls/edit/@authorized = 'true']">
					...
					<div class="sls-bo-form-page-langs sls-form-page-children-section">
						...
						<xsl:if test="//View/page/children/child[$childPosition]/urls/add/@authorized = 'true' and count(columns/column[html_type = 'input_file']) &gt; 0">
							...
							<xsl:if test="$isDefaultLang or (not($isDefaultLang) and count(//View/page/children/child[$childPosition]/columns/column[multilanguage = 'true']) &gt; 0)">
								<div class="sls-bo-form-page-lang" sls-lang="{$lang}" sls-default-lang="{$isDefaultLang}">
									...
									<xsl:if test="count(//View/page/children/child[$childPosition]/columns/column[((multilanguage = 'false' and $isDefaultLang) or multilanguage = 'true')]) &gt; 0">
										<xsl:call-template name="childBlockEdit">
										...
										<xsl:if test="//View/page/children/child[$childPosition]/urls/edit/@authorized = 'true' and //View/page/children/child[$childPosition]/model/nbChildren &gt; 0">
											<xsl:call-template name="childBlockEdit">
												<xsl:with-param name="xPath" select="concat('//View/page/children/child[', $childPosition, ']')" />
												<xsl:with-param name="isDefaultLang" select="$isDefaultLang" />
												<xsl:with-param name="lang" select="$lang" />
									...
									<xsl:if test="//View/page/children/child[$childPosition]/urls/add/@authorized = 'true'">
				...
				<div class="sls-bo-form-page-bottom">
					<div class="submit-block">
						<div class="submit-block-left">
							<div class="sls-bo-form-page-submit sls-bo-color">|||sls:lang:SLS_BO_GENERIC_SUBMIT_EDIT|||</div>
						</div>
						<div class="submit-block-right">
							<div class="select generic-select-mode sls-bo-color-border sls-bo-color-text">
								<select name="redirect">
									<option value="list">
										<xsl:if test="count(//Statics/Site/BoMenu/admin/settings[setting[@key='edit_callback']]/setting) = 0 or //Statics/Site/BoMenu/admin/settings/setting[@key='edit_callback'] = 'list'"><xsl:attribute name="selected">selected</xsl:attribute></xsl:if>
										|||sls:lang:SLS_BO_GENERIC_SUBMIT_FORWARD_LIST|||
									</option>
									<option value="edit">
										<xsl:if test="//Statics/Site/BoMenu/admin/settings/setting[@key='edit_callback'] = 'edit'"><xsl:attribute name="selected">selected</xsl:attribute></xsl:if>
	...
	<xsl:template name="childBlockEdit">
		...
		<div class="sls-bo-form-page-section sls-bo-form-page-child" sls-db="{dyn:evaluate($xPath)/model/db}" sls-model="{dyn:evaluate($xPath)/model/table}">
			<xsl:if test="$recordPosition != false()"><xsl:attribute name="sls-recordset-id"><xsl:value-of select="dyn:evaluate($xPath)/columns/column[pk = 'true' and name != 'pk_lang']/values/record[$recordPosition]/value" /></xsl:attribute></xsl:if>
			...
			<xsl:if test="$recordPosition != false()">
				<input type="hidden" name="{concat(//View/page/model/table, '[children][', dyn:evaluate($xPath)/model/table, '][', $attBlockNumber, '][properties][', $attLang,'][', dyn:evaluate($xPath)/columns/column[pk = 'true' and name != 'pk_lang']/name, ']')}" value="{dyn:evaluate($xPath)/columns/column[pk = 'true' and name != 'pk_lang']/values/record[$recordPosition]/value}" />
			</xsl:if>
			<div class="sls-bo-form-page-section-wrapper">
				<div class="sls-bo-form-page-section-content">
					...
					<xsl:for-each select="dyn:evaluate($xPath)/columns/column[pk = 'false' and ((multilanguage = 'false' and $isDefaultLang) or multilanguage = 'true')]">
						<xsl:variable name="table" select="table" />
											...
											<div class="sls-input-file-drop-zone" onclick="var rendering = this.getParent('.sls-input-file').getElement('.sls-drop-zone-rendering'); if (!Browser.ie8 &amp;&amp; ((rendering &amp;&amp; !this.getParent('.sls-form-page-field-error')) || !rendering))$('{$fieldId}').click();">
												<xsl:if test="$fieldValue != ''">
													<div class="sls-input-file-params">
														<input type="hidden" name="{$fieldName}[file]" value="{$fieldValue}">
															<xsl:choose>
																<xsl:when test="values/record[$recordPosition]/value[@lang = $lang]/@mime != '' and values/record[$recordPosition]/value[@lang = $lang]/@img != 'true'"><xsl:attribute name="sls-input-poster"><xsl:value-of select="values/record[$recordPosition]/value[@lang = $lang]/@mime" /></xsl:attribute></xsl:when>
																<xsl:otherwise><xsl:attribute name="sls-input-poster"><xsl:value-of select="$fieldValue" /></xsl:attribute></xsl:otherwise>
															</xsl:choose>
														</input>
													</div>
												...
												<input type="file" name="{$fieldName}" id="{$fieldId}" sls-input-file-value="{$fieldValue}" sls-type="{specific_type}" sls-type-extended="{specific_type_extended}" sls-file-uid="{file_uid}" sls-required="{required}" onchange="if (this.retrieve('FileUpload')) FileUpload.prototype.submit.call(this.retrieve('FileUpload')); if (Browser.ie8)this.parentNode.submit();">
													<xsl:if test="specific_type_extended = 'img'">
														...
														<xsl:if test="image_min_height != '*'"><xsl:attribute name="sls-image-min-height"><xsl:value-of select="image_min_height" /></xsl:attribute></xsl:if>
														<xsl:if test="image_ratio != '*'"><xsl:attribute name="sls-image-ratio"><xsl:value-of select="image_ratio" /></xsl:attribute></xsl:if>
														<xsl:if test="values/record[$recordPosition]/value[@lang = $lang]/@size != ''"><xsl:attribute name="sls-input-file-size"><xsl:value-of select="values/record[$recordPosition]/value[@lang = $lang]/@size" /></xsl:attribute></xsl:if>
													</xsl:if>
												</input>
												<div class="layer sls-drop-zone-caption">
													<xsl:choose>
				...
				<xsl:if test="dyn:evaluate($xPath)/urls/delete/@authorized = 'true' or ($recordPosition = false() and dyn:evaluate($xPath)/urls/delete/@authorized = 'false')">
					<div class="border bottom sls-bo-color-hover sls-bo-color-trigger">
						<div class="child-delete" sls-async-action="{concat(dyn:evaluate($xPath)/urls/delete, dyn:evaluate($xPath)/columns/column[pk = 'true' and name != 'pk_lang']/values/record[$recordPosition]/value)}">
